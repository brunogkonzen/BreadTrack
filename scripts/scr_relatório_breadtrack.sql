...
--2) Relação de produtos(nome e descrição) vendidos nos meses pares de 2022. Ordene o relatório pelo nome do produto de forma ascendente;
SELECT p.nompro, e.proenc
FROM Produto p
INNER JOIN Encomenda e ON p.codpro = e.codpro
WHERE EXTRACT(YEAR FROM e.datenc) = 2023
 -- AND EXTRACT(MONTH FROM e.datenc) % 2 = 0
ORDER BY p.nompro ASC;
